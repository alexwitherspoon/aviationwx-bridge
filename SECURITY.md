...
| Log tail allocation | `/api/logs?tail=` is capped at 1000 lines in the handler and in `GetLast`. The buffer builds the tail with bounded `append` after clamping (no `make` sized from the request), which satisfies CodeQL `go/uncontrolled-allocation-size`. |
