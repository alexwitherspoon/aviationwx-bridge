...
- Missing station time skips weather POST (Davis missing `ts`; interceptor missing/unparsable `dateutc`) - no bridge-clock `observed_at`. Emit ceiling ≤1 Hz. Failed weather POSTs stay in an in-memory retry ring for up to **10 minutes** by station `observed_at` (FIFO flush when the API recovers); no disk weather queue.
