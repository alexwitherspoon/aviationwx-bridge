...
- Missing station time skips weather POST (Davis missing `ts`; interceptor missing/unparsable `dateutc`) - no bridge-clock `observed_at`. Emit ceiling ≤1 Hz. Failed weather POSTs stay in an in-memory retry ring for up to **10 minutes** by station `observed_at` (FIFO; catch-up paced at ≤1 Hz per `source_id`, separate from live - worst case ~2 POSTs/s/station; far-future stamps beyond core skew are dropped; SoftMax is a Pi memory backstop that can drop older in-window samples if clocks stall or many stations emit near 1 Hz for the full window); no disk weather queue. This is observation catch-up, not camera image replay - stale webcam frames still must never upload.
